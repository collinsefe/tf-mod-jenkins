...
>>>>>>> c92ecb1affeab50f8e52b8dc4129bc01b8ee76ee
...
<<<<<<< HEAD
data "template_file" "ec2_jenkins_policy" {
  template = "${file("ec2_policy.json")}"
}
...
resource "aws_alb" "jenkins_alb" {
  name            = "jenkins-alb"
<<<<<<< HEAD
  subnets         = ["${data.terraform_remote_state.vpc_state.private_subnet_ids}"]
=======
...
<<<<<<< HEAD
  ...
  policy = "${data.template_file.ec2_jenkins_policy.rendered}"
}
=======
  name = "jenkins-ec2-policy"
  role = "${aws_iam_role.jenkins_ec2_role.id}"
